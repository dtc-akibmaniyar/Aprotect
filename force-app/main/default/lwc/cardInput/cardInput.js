...
export default class CardInput extends LightningElement {
    ...
    if (
      ...
      const detail = {
        ...
        value: {
          ...
          cardExpiry: this.cardExpiry.split('/').length === 2 && parseInt(this.cardExpiry.split('/')[1]) < 100 ? this.cardExpiry.split('/')[0] + '/' + (parseInt(this.cardExpiry.split('/')[1]) < 30 ? 2000 + parseInt(this.cardExpiry.split('/')[1]) : 2000 + parseInt(this.cardExpiry.split('/')[1])) : this.cardExpiry,
  ...
  despatchCompleteEvent(cardData) {
    debugger;
    const changeEvent = new CustomEvent("cardcomplete", { detail: cardData, bubbles: true, composed: true });
    this.dispatchEvent(changeEvent);
  }

  despatchIncompleteEvent() {
    const changeEvent = new CustomEvent("cardincomplete", { detail: {}, bubbles: true, composed: true });
    this.dispatchEvent(changeEvent);
  }
